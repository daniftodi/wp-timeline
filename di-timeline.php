<?php
/**
 * Plugin Name: WP Timline
 * Plugin URI: http://dan.iftodi.com
 * Description: Display a listing of posts in a timeline interface
 * Version: 0.1
 * Author: Dan Iftodi
 * Author URI: http://dan.iftodi.com
 *
 ...
 * @package WP Timeline
 * @version 0.1
 * @author Dan Iftodi <dan@iftodi.com>
 * @copyright Copyright (c) 2017, Dan Iftodi
 * @link http://dan.iftodi.com
 * @license http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 */

require_once (plugin_dir_path(__FILE__) . "./vendor/autoload.php");

add_action('wp_enqueue_scripts', 'di_template_scripts');
// Create the shortcode
add_shortcode( 'wp-timeline', 'be_display_posts_shortcode' );

function di_template_scripts() {   
    wp_enqueue_script( 'hammerjs', plugins_url( '/js/hammer.js', __FILE__ ));
}
...
function be_display_posts_shortcode( $atts ) {
	...
	$timelineTemplate = file_get_contents(plugin_dir_path(__FILE__) . "./templates/timeline.ditemplate");
	$itemTemplate = file_get_contents(plugin_dir_path(__FILE__) . "./templates/item.ditemplate");
	...
	while ( $listing->have_posts() ): $listing->the_post(); global $post;
		...
		$output = $mustache->render($itemTemplate, [
				'time' => get_the_date(),
				'title' => $title,
				'content' => $excerpt
			]);
	...
	$close = "<style type='text/css'>" . file_get_contents(plugin_dir_path(__FILE__) . "/css/style.css") . "</style>";
	$close .= "<script type='text/javascript'>" . file_get_contents(plugin_dir_path(__FILE__) . "/js/scripts.js") . "</script>";
	...
	$return = $mustache->render($timelineTemplate, [
			'title' => 'Timeline',
			'content' => $open . $inner,
			'style' => $close
		]);
